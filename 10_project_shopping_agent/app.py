...
if prompt := st.chat_input("e.g. I want organic honey under $15 with 4+ rating"):
    ...
    has_conversation = any(
        message["role"] == "assistant" for message in st.session_state.messages
    )
    if not is_shopping_related(prompt, allow_confirmation=has_conversation):
        response = (
            "I can only help with shopping, product searches, orders, "
